...
export default function ConnectionsView({
  ...
  // Unified Mode State
  const [uniHost, setUniHost] = useState("100.96.62.57");
  const [uniPort, setUniPort] = useState("22");
  const [uniUser, setUniUser] = useState("ammar");
  const [uniPass, setUniPass] = useState("ammar@1234");
  const [uniAuth, setUniAuth] = useState("Password");
  const [uniSshKey, setUniSshKey] = useState("");
  const [uniTrust, setUniTrust] = useState(true);
  const [uniDir837, setUniDir837] = useState("/hello");
  const [uniDir835, setUniDir835] = useState("/835");
  const [uniDirMir, setUniDirMir] = useState("/mir");
  ...
  const [inHost, setInHost] = useState("100.96.62.57");
  const [inPort, setInPort] = useState("22");
  const [inUser, setInUser] = useState("ammar");
  const [inPass, setInPass] = useState("ammar@1234");
  const [inAuth, setInAuth] = useState("Password");
  const [inDir837, setInDir837] = useState("/hello");
  const [inDir835, setInDir835] = useState("/835");
  const [showInPass, setShowInPass] = useState(false);

  // Dual Mode State - Outbound
  const [outHost, setOutHost] = useState("100.96.62.57");
  const [outPort, setOutPort] = useState("22");
  const [outUser, setOutUser] = useState("ammar");
  const [outPass, setOutPass] = useState("ammar@1234");
  const [outAuth, setOutAuth] = useState("Password");
  const [outDirMir, setOutDirMir] = useState("/mir");
